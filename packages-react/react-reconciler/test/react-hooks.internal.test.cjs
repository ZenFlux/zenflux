...
describe( 'ReactHooks', () => {
    ...
    // Edge case.
    // Not working in the original source code.
    it.skip( 'warns when reading context inside eager useReducer', () => {
    ...
    it( 'warns when calling hooks inside useReducer', () => {
        ...
        expect( () => {
            ...
            'Do not call Hooks inside useEffect(...), useMemo(...), or other built-in Hooks',
            'React has detected a change in the order of Hooks called by App. This will lead to bugs and errors if not fixed. For more information, read the Rules of Hooks: https://reactjs.org/link/rules-of-hooks\n\n' +
    ...
    it( 'double-invokes components with Hooks in Strict Mode', () => {
        ...
        if ( ! require( '@zenflux/react-shared/src/react-feature-flags' ).disableModulePatternComponents ) {
            renderCount = 0;
            expect( () => renderer.update( <Factory/> ) ).toErrorDev(
                'The <Factory /> component appears to be a function component that returns a class instance. ' +
    ...
    it( 'does not swallow original error when updating another component in render phase', async () => {
        ...
        if ( __DEV__ ) {
            ...
            expect( console.error.mock.calls[ 0 ][ 0 ] ).toContain(
                'Cannot update a component (`%s`) while rendering ' +
