...
import { createFiberFromFragment, } from "@zenflux/react-reconciler/src/react-fiber";
...
} from "@zenflux/react-reconciler/src/react-fiber-create-utils";
import {
...
import type { CacheComponentState, SpawnedCachePool } from "@zenflux/react-shared/src/react-internal-types/cache";

import type { LazyComponent as LazyComponentType } from "@zenflux/react-shared/src/lazy-component";

import type { Transition, TransitionStatus } from "@zenflux/react-shared/src/react-internal-types/transition";
    ...
    if ( current !== null ) {
        ...
        if ( shallowEqual( prevProps, nextProps ) && current.ref === workInProgress.ref && ( // Prevent bailout if the implementation changed due to hot reload.
            ...
            if ( ! checkScheduledUpdateOrContext( current, renderLanes ) ) {
                ...
                return bailoutOnAlreadyFinishedWork( current, workInProgress, renderLanes );
            } else if ( ( current.flags & FiberFlags.ForceUpdateForLegacySuspense ) !== FiberFlags.NoFlags ) {
...
function updateOffscreenComponent( current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes ) {
    ...
    if ( nextProps.mode === "hidden" || enableLegacyHidden && nextProps.mode === "unstable-defer-without-hiding" || nextIsDetached ) {
        ...
        const didSuspend = ( workInProgress.flags & FiberFlags.DidCapture ) !== FiberFlags.NoFlags;
    ...
    if ( current === null ) {
        ...
        if ( prevState.parent !== parentCache ) {
            ...
            if ( workInProgress.lanes === NoLanes ) {
                const updateQueue: FiberUpdateQueue<any> = ( workInProgress.updateQueue as any );
...
function markRef( current: Fiber | null, workInProgress: Fiber ) {
    ...
    if ( current === null && ref !== null || current !== null && current.ref !== ref ) {
        ...
        workInProgress.flags |= FiberFlags.Ref;
        workInProgress.flags |= FiberFlags.RefStatic;
    }
}
...
function finishClassComponent( current: Fiber | null, workInProgress: Fiber, Component: any, shouldUpdate: boolean, hasContext: boolean, renderLanes: Lanes ) {
    ...
    markRef( current, workInProgress );
    const didCaptureError = ( workInProgress.flags & FiberFlags.DidCapture ) !== FiberFlags.NoFlags;
    ...
    if ( didCaptureError && typeof Component.getDerivedStateFromError !== "function" ) {
        ...
        if ( __DEV__ ) {
            ...
            if ( debugRenderPhaseSideEffectsForStrictMode && workInProgress.mode & TypeOfMode.StrictLegacyMode ) {
...
function updateHostRoot( current: null | Fiber, workInProgress: Fiber, renderLanes: Lanes ) {
    ...
    if ( supportsHydration && prevState.isDehydrated ) {
        ...
        const updateQueue: FiberUpdateQueue<RootState> = ( workInProgress.updateQueue as any );
        ...
        if ( workInProgress.flags & FiberFlags.ForceClientRender ) {
            ...
            while ( node ) {
                ...
                node.flags = node.flags & ~FiberFlags.Placement | FiberFlags.Hydrating;
                node = node.sibling;
            }
        }
    } else {
...
function mountHostRootWithoutHydrating( current: Fiber, workInProgress: Fiber, nextChildren: ReactNodeList, renderLanes: Lanes, recoverableError: CapturedValue<unknown> ) {
    ...
    workInProgress.flags |= FiberFlags.ForceClientRender;
    reconcileChildren( current, workInProgress, nextChildren, renderLanes );
    return workInProgress.child;
}
...
function mountIndeterminateComponent( _current: null | Fiber, workInProgress: Fiber, Component: any, renderLanes: Lanes ) {
    ...
    if ( __DEV__ ) {
        ...
        if ( workInProgress.mode & TypeOfMode.StrictLegacyMode ) {
...
function mountSuspenseFallbackChildren( workInProgress: Fiber, primaryChildren: any, fallbackChildren: any, renderLanes: Lanes ) {
    ...
    if ( ( mode & TypeOfMode.ConcurrentMode ) === TypeOfMode.NoMode && progressedPrimaryFragment !== null ) {
        ...
        if ( enableProfilerTimer && workInProgress.mode & TypeOfMode.ProfileMode ) {
...
function updateSuspensePrimaryChildren( current: Fiber, workInProgress: Fiber, primaryChildren: any, renderLanes: Lanes ) {
    ...
    if ( currentFallbackChildFragment !== null ) {
        ...
        if ( deletions === null ) {
            ...
            workInProgress.flags |= FiberFlags.ChildDeletion;
        } else {
...
function updateSuspenseFallbackChildren( current: Fiber, workInProgress: Fiber, primaryChildren: any, fallbackChildren: any, renderLanes: Lanes ) {
    ...
    if ( // In legacy mode, we commit the primary tree as if it successfully
        // completed, even though it's in an inconsistent state.
        ( mode & TypeOfMode.ConcurrentMode ) === TypeOfMode.NoMode && // Make sure we're on the second pass, i.e. the primary child fragment was
        ...
        primaryChildFragment.subtreeFlags = currentPrimaryChildFragment.subtreeFlags & FiberFlags.StaticMask;
    }
    ...
    if ( currentFallbackChildFragment !== null ) {
        ...
        fallbackChildFragment.flags |= FiberFlags.Placement;
    }
...
function retrySuspenseComponentWithoutHydrating( current: Fiber, workInProgress: Fiber, renderLanes: Lanes, recoverableError: CapturedValue<unknown> | null ) {
    ...
    // Needs a placement effect because the parent (the Suspense boundary) already
    // mounted but this is a new fiber.
    primaryChildFragment.flags |= FiberFlags.Placement;
    workInProgress.memoizedState = null;
    return primaryChildFragment;
}
...
function updateDehydratedSuspenseComponent( current: Fiber, workInProgress: Fiber, didSuspend: boolean, nextProps: any, suspenseInstance: SuspenseInstance, suspenseState: SuspenseState, renderLanes: Lanes ): null | Fiber {
    if ( ! didSuspend ) {
        ...
        if ( didWorkInProgressReceiveUpdate() || hasContextChanged ) {
            ...
            // inserted into the React tree here. It just happens to not need DOM
            // mutations because it already exists.
            primaryChildFragment.flags |= FiberFlags.Hydrating;
            return primaryChildFragment;
        }
    } else {
...
function updateSuspenseListComponent( current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes ) {
    ...
    if ( shouldForceFallback ) {
        ...
        const didSuspendBefore = current !== null && ( current.flags & FiberFlags.DidCapture ) !== FiberFlags.NoFlags;
...
function remountFiber( current: Fiber, oldWorkInProgress: Fiber, newWorkInProgress: Fiber ): Fiber | null {
    if ( __DEV__ ) {
        ...
        newWorkInProgress.flags |= FiberFlags.Placement;
...
function attemptEarlyBailoutIfNoScheduledUpdate( current: Fiber, workInProgress: Fiber, renderLanes: Lanes ) {
    ...
    switch ( workInProgress.tag ) {
        ...
        case WorkTag.SuspenseListComponent: {
            const didSuspendBefore = ( current.flags & FiberFlags.DidCapture ) !== FiberFlags.NoFlags;
...
function beginWork( current: Fiber | null, workInProgress: Fiber, renderLanes: Lanes ): Fiber | null {
    ...
    if ( current !== null ) {
        ...
        if ( oldProps !== newProps || hasLegacyContextChanged() || ( // Force a re-render if the implementation changed due to hot reload:
            ...
            if ( ( current.flags & FiberFlags.ForceUpdateForLegacySuspense ) !== FiberFlags.NoFlags ) {
                // This is a special case that only exists for legacy mode.
                // See https://github.com/facebook/react/pull/19216.
                markWorkInProgressReceivedUpdate();
            } else {
