    ...
    if ( mixedRef !== null && typeof mixedRef !== "function" && typeof mixedRef !== "object" ) {
        if ( __DEV__ ) {
            if ( // We warn in ReactElement.js if owner and self are equal for string refs
                ...
                ! ( element._owner && ( ( element._owner as any ) as Fiber ).tag !== WorkTag.ClassComponent ) && // Will already warn with "Function components cannot be given refs"
...
function createChildReconciler( shouldTrackSideEffects: boolean ): ChildReconciler {
    ...
    function placeChild( newFiber: Fiber, lastPlacedIndex: number, newIndex: number ): number {
        ...
        if ( ! shouldTrackSideEffects ) {
            ...
            newFiber.flags |= FiberFlags.Forked;
        ...
        if ( current !== null ) {
            ...
            if ( oldIndex < lastPlacedIndex ) {
                // This is a move.
                newFiber.flags |= FiberFlags.Placement | FiberFlags.PlacementDEV;
                return lastPlacedIndex;
            } else {
    ...
    function updatePortal( returnFiber: Fiber, current: Fiber | null, portal: ReactPortal, lanes: Lanes ): Fiber {
        if ( current === null || current.tag !== WorkTag.HostPortal || current.stateNode.containerInfo !== portal.containerInfo || current.stateNode.implementation !== portal.implementation ) {
            // Insert
    ...
    function reconcileSinglePortal( returnFiber: Fiber, currentFirstChild: Fiber | null, portal: ReactPortal, lanes: Lanes ): Fiber {
        ...
        while ( child !== null ) {
            ...
            // the first item in the list.
            if ( child.key === key ) {
                if ( child.tag === WorkTag.HostPortal && child.stateNode.containerInfo === portal.containerInfo && child.stateNode.implementation === portal.implementation ) {
                    deleteRemainingChildren( returnFiber, child.sibling );
