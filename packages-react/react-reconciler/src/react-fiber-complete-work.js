...
function markRef(workInProgress) {
    workInProgress.flags |= fiber_flags_1.FiberFlags.Ref | fiber_flags_1.FiberFlags.RefStatic;
}
function hadNoMutationsEffects(current, completedWork) {
    ...
    if ((completedWork.flags & fiber_flags_1.FiberFlags.ChildDeletion) !== fiber_flags_1.FiberFlags.NoFlags) {
        return false;
    }
    ...
    while (child !== null) {
        if ((child.flags & fiber_flags_1.FiberFlags.MutationMask) !== fiber_flags_1.FiberFlags.NoFlags || (child.subtreeFlags & fiber_flags_1.FiberFlags.MutationMask) !== fiber_flags_1.FiberFlags.NoFlags) {
            return false;
        }
        child = child.sibling;
    }
...
function appendAllChildren(parent, workInProgress, needsVisibilityToggle, isHidden) {
    if (supportsMutation) {
        ...
        while (node !== null) {
            if (node.tag === work_tags_1.WorkTag.HostComponent || node.tag === work_tags_1.WorkTag.HostText) {
                appendInitialChild(parent, node.stateNode);
            }
            else if (node.tag === work_tags_1.WorkTag.HostPortal || (react_feature_flags_1.enableHostSingletons && supportsSingletons ? node.tag === work_tags_1.WorkTag.HostSingleton : false)) { // If we have a portal child, then we don't want to traverse
        ...
        while (node !== null) {
            ...
            else if (node.tag === work_tags_1.WorkTag.HostPortal) { // If we have a portal child, then we don't want to traverse
                // down its children. Instead, we'll get insertions from each child in
                // the portal directly.
            }
            else if (node.tag === work_tags_1.WorkTag.OffscreenComponent && node.memoizedState !== null) {
...
function preloadInstanceAndSuspendIfNeeded(workInProgress, type, props, renderLanes) {
    if (!maySuspendCommit(type, props)) {
        ...
        workInProgress.flags &= ~fiber_flags_1.FiberFlags.MaySuspendCommit;
        return;
    }
    ...
    if (!(0, fiber_lane_constants_1.includesOnlyNonUrgentLanes)(rootRenderLanes)) { // This is an urgent render. Don't suspend or show a fallback. Also,
        ...
        if (!isReady) {
            if ((0, react_fiber_work_loop_should_on_previous_screen_1.shouldRemainOnPreviousScreen)()) {
                ...
                workInProgress.flags |= fiber_flags_1.FiberFlags.ShouldSuspendCommit;
            }
            else {
...
function preloadResourceAndSuspendIfNeeded(workInProgress, resource, type, props, renderLanes) {
    ...
    workInProgress.flags |= fiber_flags_1.FiberFlags.MaySuspendCommit;
    var rootRenderLanes = (0, react_fiber_work_in_progress_1.getWorkInProgressRootRenderLanes)();
    if (!(0, fiber_lane_constants_1.includesOnlyNonUrgentLanes)(rootRenderLanes)) { // This is an urgent render. Don't suspend or show a fallback.
    }
    else {
...
function scheduleRetryEffect(workInProgress, retryQueue) {
    ...
    if (wakeables !== null) {
        ...
        if (workInProgress.flags & fiber_flags_1.FiberFlags.ScheduleRetry) {
            ...
            // and retry lanes.
            workInProgress.tag !== work_tags_1.WorkTag.OffscreenComponent ? (0, react_fiber_lane_1.claimNextRetryLane)() : fiber_lane_constants_1.OffscreenLane;
...
function bubbleProperties(completedWork) {
    ...
    var newChildLanes = fiber_lane_constants_1.NoLanes;
    var subtreeFlags = fiber_flags_1.FiberFlags.NoFlags;
    if (!didBailout) {
        ...
        if (react_feature_flags_1.enableProfilerTimer && (completedWork.mode & type_of_mode_1.TypeOfMode.ProfileMode) !== type_of_mode_1.TypeOfMode.NoMode) {
            ...
            while (child !== null) {
                ...
                // ignore them.
                subtreeFlags |= child.subtreeFlags & fiber_flags_1.FiberFlags.StaticMask;
                subtreeFlags |= child.flags & fiber_flags_1.FiberFlags.StaticMask;
                // $FlowFixMe[unsafe-addition] addition with possible null/undefined value
                // @ts-ignore
                treeBaseDuration += child.treeBaseDuration;
                child = child.sibling;
...
function completeDehydratedSuspenseBoundary(current, workInProgress, nextState) {
    if ((0, react_fiber_hydration_context_1.hasUnhydratedTailNodes)() && (workInProgress.mode & type_of_mode_1.TypeOfMode.ConcurrentMode) !== type_of_mode_1.TypeOfMode.NoMode && (workInProgress.flags & fiber_flags_1.FiberFlags.DidCapture) === fiber_flags_1.FiberFlags.NoFlags) {
        ...
        workInProgress.flags |= fiber_flags_1.FiberFlags.ForceClientRender | fiber_flags_1.FiberFlags.DidCapture;
        return false;
    }
    ...
    if (nextState !== null && nextState.dehydrated !== null) {
        ...
        if (current === null) {
            ...
            if (react_feature_flags_1.enableProfilerTimer) {
                if ((workInProgress.mode & type_of_mode_1.TypeOfMode.ProfileMode) !== type_of_mode_1.TypeOfMode.NoMode) {
...
function completeWork(current, workInProgress, renderLanes) {
    ...
    switch (workInProgress.tag) {
        ...
        case work_tags_1.WorkTag.HostRoot: {
                ...
                if (wasHydrated) {
                    ...
                    if (current !== null) {
                        ...
                        !prevState.isDehydrated || // Check if we reverted to client rendering (e.g. due to an error)
                            (workInProgress.flags & fiber_flags_1.FiberFlags.ForceClientRender) !== fiber_flags_1.FiberFlags.NoFlags) {
            ...
            bubbleProperties(workInProgress);
            if (react_feature_flags_1.enableTransitionTracing) {
                if ((workInProgress.subtreeFlags & fiber_flags_1.FiberFlags.Visibility) !== fiber_flags_1.FiberFlags.NoFlags) {
        ...
        case work_tags_1.WorkTag.SuspenseComponent: {
            ...
            if (react_feature_flags_1.enableCache && nextDidTimeout) {
                ...
                if (cache !== previousCache) {
                    // Run passive effects to retain/release the cache.
                    offscreenFiber.flags |= fiber_flags_1.FiberFlags.Passive;
                }
            }
            ...
            if (nextDidTimeout !== prevDidTimeout) {
                ...
                if (nextDidTimeout) {
                    var offscreenFiber = workInProgress.child;
                    offscreenFiber.flags |= fiber_flags_1.FiberFlags.Visibility;
                }
            }
        ...
        case work_tags_1.WorkTag.SuspenseListComponent: {
            ...
            var didSuspendAlready = (workInProgress.flags & fiber_flags_1.FiberFlags.DidCapture) !== fiber_flags_1.FiberFlags.NoFlags;
            ...
            if (renderedTail === null) {
                ...
                if (!didSuspendAlready) {
                    ...
                    // findFirstSuspended.
                    var cannotBeSuspended = (0, react_fiber_work_in_progress_ping_1.renderHasNotSuspendedYet)() && (current === null || (current.flags & fiber_flags_1.FiberFlags.DidCapture) === fiber_flags_1.FiberFlags.NoFlags);
                ...
                if (!didSuspendAlready) {
                    ...
                    // exceed it.
                    (0, react_scheduler_1.unstable_now)() * 2 - renderState.renderingStartTime > (0, react_fiber_work_loop_1.getRenderTargetTime)() && renderLanes !== fiber_lane_constants_1.OffscreenLane) {
        ...
        case work_tags_1.WorkTag.LegacyHiddenComponent: {
            ...
            if (react_feature_flags_1.enableLegacyHidden && workInProgress.tag === work_tags_1.WorkTag.LegacyHiddenComponent) { // LegacyHidden doesn't do any hiding — it only pre-renders.
            }
            else {
                if (current !== null) {
                    var prevState = current.memoizedState;
            ...
            if (!nextIsHidden || (workInProgress.mode & type_of_mode_1.TypeOfMode.ConcurrentMode) === type_of_mode_1.TypeOfMode.NoMode) {
                bubbleProperties(workInProgress);
            }
            else {
                // Don't bubble properties for hidden children unless we're rendering
                // at offscreen priority.
                if ((0, react_fiber_lane_1.includesSomeLane)(renderLanes, fiber_lane_constants_1.OffscreenLane) && // Also don't bubble if the tree suspended
                    (workInProgress.flags & fiber_flags_1.FiberFlags.DidCapture) === fiber_lane_constants_1.NoLanes) {
                    bubbleProperties(workInProgress);
                    // Check if there was an insertion or update in the hidden subtree.
                    // If so, we need to hide those nodes in the commit phase, so
                    // schedule a visibility effect.
                    if ((!react_feature_flags_1.enableLegacyHidden || workInProgress.tag !== work_tags_1.WorkTag.LegacyHiddenComponent) && workInProgress.subtreeFlags & (fiber_flags_1.FiberFlags.Placement | fiber_flags_1.FiberFlags.Update)) {
                        workInProgress.flags |= fiber_flags_1.FiberFlags.Visibility;
                    }
                }
            }
