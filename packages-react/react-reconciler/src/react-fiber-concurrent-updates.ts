...
export function enqueueConcurrentHookUpdate<S, A>( fiber: Fiber, queue: HookUpdateQueue<S, A>, update: HookUpdate<S, A>, lane: Lane ): FiberRoot | null {
...
export function enqueueConcurrentHookUpdateAndEagerlyBailout<S, A>( fiber: Fiber, queue: HookUpdateQueue<S, A>, update: HookUpdate<S, A> ): void {
...
export function enqueueConcurrentClassUpdate<State>( fiber: Fiber, queue: SharedQueue<State>, update: FiberUpdate<State>, lane: Lane ): FiberRoot | null {
    const concurrentQueue: ConcurrentQueue = ( queue as any );
    const concurrentUpdate: ConcurrentUpdate = ( update as any );
    enqueueUpdate( fiber, concurrentQueue, concurrentUpdate, lane );
    return getRootForUpdatedFiber( fiber );
...
function markUpdateLaneFromFiberToRoot( sourceFiber: Fiber, update: ConcurrentUpdate | null, lane: Lane ): void {
    ...
    if ( isHidden && update !== null && node.tag === WorkTag.HostRoot ) {
...
function getRootForUpdatedFiber( sourceFiber: Fiber ): FiberRoot | null {
    ...
    return node.tag === WorkTag.HostRoot ? ( node.stateNode as FiberRoot ) : null;
}
...
function warnAboutUpdateOnNotYetMountedFiberInDEV( fiber: Fiber ) {
    if ( __DEV__ ) {
        ...
        if ( tag !== WorkTag.IndeterminateComponent && tag !== WorkTag.HostRoot && tag !== WorkTag.ClassComponent && tag !== WorkTag.FunctionComponent && tag !== WorkTag.ForwardRef && tag !== WorkTag.MemoComponent && tag !== WorkTag.SimpleMemoComponent ) {
...
function detectUpdateOnUnmountedFiber( sourceFiber: Fiber, parent: Fiber ) {
    if ( __DEV__ ) {
        ...
        if ( alternate === null && ( parent.flags & ( FiberFlags.Placement | FiberFlags.Hydrating ) ) !== FiberFlags.NoFlags ) {
