...
export const SuspenseException: unknown = new Error( "Suspense Exception: This is not a real error! It's an implementation " + "detail of `use` to interrupt the current render. You must either " + "rethrow it immediately, or move the `use` call outside of the " + "`try/catch` block. Capturing without rethrowing will lead to " + "unexpected behavior.\n\n" + "To handle async errors, wrap your component in an error boundary, or " + "call the promise's `.catch` method and pass the result to `use`" );
export const SuspenseyCommitException: unknown = new Error( "Suspense Exception: This is not a real error, and should not leak into " + "userspace. If you're seeing this, it's likely a bug in React." );
...
export function trackUsedThenable<T>( thenableState: ThenableState, thenable: Thenable<T>, index: number ): T {
    ...
    switch ( thenable.status ) {
        ...
        default: {
            if ( typeof thenable.status === "string" ) {
                ...
                // @ts-ignore
                switch ( thenable.status as string ) {
                    case "fulfilled": {
                        const fulfilledThenable: FulfilledThenable<T> = ( thenable as any );
