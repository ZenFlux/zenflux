...
function propagateParentContextChanges( current: Fiber, workInProgress: Fiber, renderLanes: Lanes, forcePropagateEntireTree: boolean ) {
    ...
    while ( parent !== null ) {
        if ( ! isInsidePropagationBailout ) {
            if ( ( parent.flags & FiberFlags.NeedsPropagation ) !== FiberFlags.NoFlags ) {
                isInsidePropagationBailout = true;
            } else if ( ( parent.flags & FiberFlags.DidPropagateContext ) !== FiberFlags.NoFlags ) {
                break;
            }
        }
    ...
    workInProgress.flags |= FiberFlags.DidPropagateContext;
}
...
function readContextForConsumer<T>( consumer: Fiber | null, context: ReactContext<T> ): T {
    ...
    if ( lastFullyObservedContext === context ) {// Nothing to do. We already observe everything in this context.
        ...
        if ( lastContextDependency === null ) {
            ...
            if ( enableLazyContextPropagation ) {
                consumer.flags |= FiberFlags.NeedsPropagation;
            }
        } else {
