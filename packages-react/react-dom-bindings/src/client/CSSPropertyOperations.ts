 ...
 * // @param {object} styles
 */
export function setValueForStyles( node: HTMLElement, styles: Record<CSSShorthandKeys, string>, prevStyles: Record<CSSShorthandKeys, string> ) {
    ...
    if ( prevStyles != null ) {
        ...
        //     }
        // }
        ( Object.keys( styles ) as CSSShorthandKeys[] ).forEach( ( styleName ) => {
            const value = styles[ styleName ];
 ...
 */
function expandShorthandMap( styles: Record<CSSShorthandKeys, string> ) {
    const expanded: any = {};

    ( Object.keys( styles ) as CSSShorthandKeys[] ).forEach( ( key ) => {
        const longhand = shorthandToLonghand[ key ] || [ key ];
 ...
 */
function validateShorthandPropertyCollisionInDev( prevStyles: Record<CSSShorthandKeys, string>, nextStyles: Record<CSSShorthandKeys, string> ) {
    if ( __DEV__ ) {
        if ( ! nextStyles ) {
            return;
        }
        ...
        const expandedUpdates: Record<string, CSSShorthandKeys> = {};
        ...
        if ( prevStyles ) {
            ...
            ( Object.keys( prevStyles ) as CSSShorthandKeys[] ).forEach( ( prop ) => {
                if ( prevStyles.hasOwnProperty( prop ) && ! nextStyles.hasOwnProperty( prop ) ) {
                    const longhands = shorthandToLonghand[ prop ] || [ prop ];
        ...
        ( Object.keys( nextStyles ) as CSSShorthandKeys[] ).forEach( ( key ) => {
        ...
        const warnedAbout: Record<string, boolean> = {};
        ...
        ( Object.keys( expandedUpdates ) as CSSShorthandKeys[] ).forEach( ( key ) => {
            const originalKey = expandedUpdates[ key ];
            const correctOriginalKey = expandedStyles[ key ];
