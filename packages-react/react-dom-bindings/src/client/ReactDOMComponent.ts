...
import { checkAttributeStringCoercion, checkHtmlStringCoercion } from "@zenflux/react-shared/src/check-string-coercion";
...
} from "@zenflux/react-dom-bindings/src/events/EventRegistry";
...
import {
    ...
    setValueForStyles
} from "@zenflux/react-dom-bindings/src/client/CSSPropertyOperations";
import { MATH_NAMESPACE, SVG_NAMESPACE } from "@zenflux/react-dom-bindings/src/client/DOMNamespaces";
import { validateProperties as validateInputProperties } from "@zenflux/react-dom-bindings/src/shared/ReactDOMNullInputValuePropHook";
import { validateProperties as validateUnknownProperties } from "@zenflux/react-dom-bindings/src/shared/ReactDOMUnknownPropertyHook";
...
import { HostContextNamespaceNone } from "@zenflux/react-dom-bindings/src/client/react-fiber-config-host-context-namespace";
...
let canDiffStyleForHydrationWarning: boolean;
...
function warnForInvalidEventListener( registrationName: keyof typeof possibleRegistrationNames, listener: any ) {
    if ( __DEV__ ) {
        if ( listener === false ) {
...
function setPropOnCustomElement( domElement: Element, tag: string, key: keyof Props, value: any, props: any, prevValue: any ): void {
    switch ( key ) {
        case "style": {
            setValueForStyles( domElement as HTMLElement, value, prevValue );
            break;
        ...
        default: {
            if ( registrationNameDependencies.hasOwnProperty( key ) ) {
                if ( __DEV__ && value != null && typeof value !== "function" ) {
                    warnForInvalidEventListener( key as keyof typeof registrationNameDependencies, value );
                }
            } else {
                if ( enableCustomElementPropertySupport ) {
                    ...
                    setValueForAttribute( domElement, key.toString(), value );
                }
            }
        }
    }
...
export function setInitialProperties( domElement: Element, tag: string, anyProps: AnyProps ): void {
    if ( __DEV__ ) {
    ...
    switch ( tag ) {
        ...
        case "input": {
            const props: Props = anyProps as Props;
            ...
            for ( const key in props ) {
                ...
                switch ( propKey ) {
                    ...
                    default: {
                        setProp( domElement, tag, propKey, props[ propKey ], props, null );
                    }
                }
            }
        ...
        case "select": {
            ...
            for ( const key in props ) {
                ...
                switch ( propKey ) {
                    ...
                    case "multiple": {
                        multiple = props[ propKey ]; // TODO: We don't actually have to fall through here because we set it
...
export function updateProperties( domElement: Element, tag: string, lastProps: Props, nextProps: Props ): void {
    if ( __DEV__ ) {
    ...
    switch ( tag ) {
            ...
            for ( const key in lastProps ) {
                ...
                if ( lastProps.hasOwnProperty( propKey ) && lastProps[ propKey ] !== null ) {
                    switch ( propKey ) {
                        ...
                        // Fallthrough
                        default: {
                            if ( ! nextProps.hasOwnProperty( propKey ) ) setProp( domElement, tag, propKey, null, nextProps, lastProps[ propKey ] );
                        }
                    }
                }
            }
        ...
        case "select": {
            ...
            let defaultValue = null;
            let multiple = null;
            let wasMultiple: boolean | null | undefined = null;
...
function getPossibleStandardName( propName: string ) {
    if ( __DEV__ ) {
        ...
        return possibleStandardNames[ lowerCasedName as keyof typeof possibleStandardNames ] || null;
    }
...
export function diffHydratedProperties( domElement: Element, tag: string, props: Props, isConcurrentMode: boolean, shouldWarnDev: boolean, hostContext: HostContext ): void {
    if ( __DEV__ ) {
