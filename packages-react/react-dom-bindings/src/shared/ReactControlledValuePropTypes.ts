...
export function checkControlledValueProps( tagName: string, props: Props ): void {
    if ( __DEV__ ) {
        if ( ! ( hasReadOnlyValue[ props.type as keyof typeof hasReadOnlyValue ] || props.onChange || props.onInput || props.readOnly || props.disabled || props.value == null ) ) {
