...
export function act<T>( callback: () => T | Thenable<T> ): Thenable<T> {
    const ReactCurrentActQueue = global.React.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactCurrentActQueue;
    ...
    if ( __DEV__ ) {
        ...
        if (
            ...
            const thenable: Thenable<T> = result as any;
...
function popActScope(
    prevActQueue: null | Array<Function>,
...
function flushActQueue( queue: Function[] ) {
    if ( __DEV__ ) {
        if ( ! isFlushing ) {
            // Prevent re-entrance.
            isFlushing = true;
